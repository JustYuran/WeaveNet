...
class Game {
    ...
    constructor() {
        ...
        // [ЧТО] Создаём гексагональную сетку размером 200×200 в форме России
        // [ЗАЧЕМ] Большое игровое поле для размещения построек
     ...
     */
    placeBuilding(hex) {
        // [ЧТО] Проверяем можно ли строить на этом гексе (преграды, биомы)
        // [ЗАЧЕМ] 1.2.3 - Логика размещения, 1.3.1 - Учёт преград рельефа
        // [PLAN] 1.3.3 - Тактическое использование ландшафта
        if (!this.hexGrid.canBuildOn(hex)) {
            console.log(`[Game] Нельзя строить на гексе ${hex.id}: препятствие или занято`);
            return;
        }
