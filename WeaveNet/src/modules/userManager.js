...
class UserManager {
    ...
    constructor(hexGrid) {
        ...
        // [ЗАЧЕМ] Отслеживание интервала обновления
        // [PLAN] Использовать игровой цикл вместо setTimeout
        this.lastUpdateTime = Date.now();
        this.updateInterval = 1300; // 1.3 секунды (увеличено на 30% для оптимизации)
        
        // [ЧТО] Шанс перемещения пользователя
        // [ЗАЧЕМ] Контроль активности перемещений
        // [PLAN] Сделать настраиваемым параметром
        this.moveChance = 0.35; // 35% (снижено на 30% для оптимизации)
        ...
        // [ЧТО] Кэш количества пользователей на гексах
        // [ЗАЧЕМ] Оптимизация производительности - избегаем многократных фильтраций
        // [PLAN] Обновлять кэш только при изменениях
        this.hexUserCountCache = {};
    ...
    createUser() {
        ...
        // [ЧТО] Инвалидируем кэш после изменения
        // [ЗАЧЕМ] Кэш должен быть пересчитан при следующем запросе
        // [PLAN] Обновлять кэш инкрементально вместо полной инвалидации
        this.isCacheValid = false;
    ...
    getAvailableHexes() {
        ...
        // [ЧТО] Подсчитываем пользователей на каждом гексе с использованием кэша
     ...
     * [ЧТО] Каждую 1.3 секунды пользователи могут случайно переместиться
     * [ЗАЧЕМ] Симуляция активности на карте с оптимизацией производительности
     * [PLAN] Добавить логику принятия решений о перемещении
     */
    update() {
        const now = Date.now();
        
        // [ЧТО] Проверяем прошёл ли интервал с последнего обновления
        // [ЗАЧЕМ] Обновление не чаще чем раз в 1.3 секунды для оптимизации
        ...
        // [ЧТО] Инвалидируем кэш перед обновлением
        // [ЗАЧЕМ] Кэш будет пересчитан при необходимости
        // [PLAN] Обновлять кэш инкрементально при изменениях
        this.isCacheValid = false;
     ...
     */
    updateUser(user) {
        // [ЧТО] Проверяем шанс перемещения (настраиваемый параметр)
        // [ЗАЧЕМ] Случайное поведение - некоторые остаются на месте
        // [PLAN] Настроить вероятность через параметры
        const shouldMove = Math.random() < this.moveChance;
        ...
        const availableNeighbors = neighborIds.filter(neighborId => {
            const countOnHex = this.getUserCountOnHex(neighborId);
     ...
     * [ЧТО] Подсчитывает пользователей на гексе с использованием кэша
     * [ЗАЧЕМ] Быстрая проверка заполненности с оптимизацией производительности
     * [PLAN] Кэшировать результаты
     * @param {number} hexId - ID гекса
     * @returns {number} Количество пользователей
     */
    getUserCountOnHex(hexId) {
        // [ЧТО] Проверяем валидность кэша
        // [ЗАЧЕМ] Используем кэш для оптимизации повторяющихся запросов
        // [PLAN] Обновлять кэш инкрементально при изменениях
        if (!this.isCacheValid) {
            this._rebuildUserCountCache();
        }
        return this.hexUserCountCache[hexId] || 0;
    }
    
    /**
     * Перестроение кэша количества пользователей на гексах
     * [ЧТО] Подсчитывает всех пользователей и обновляет кэш
     * [ЗАЧЕМ] Оптимизация множественных вызовов getUserCountOnHex
     * [PLAN] Вызывать только при изменениях в массиве пользователей
     */
    _rebuildUserCountCache() {
        this.hexUserCountCache = {};
        this.users.forEach(user => {
            if (!this.hexUserCountCache[user.hexId]) {
                this.hexUserCountCache[user.hexId] = 0;
            }
            this.hexUserCountCache[user.hexId]++;
        });
     ...
     * [ЧТО] Находит и возвращает пользователя по уникальному ID
