...
class HexGrid {
    ...
    createHex(q, r, id) {
        ...
        // [ЧТО] Генерируем преграды рельефа (горы, пропасти, вода)
        // [ЗАЧЕМ] Тактические элементы для стратегического геймплея
        // [PLAN] 1.3.1 - Реализовать типы непроходимости
        
        const terrain = this.getTerrainForHex(q, r);
        const obstacle = this.generateObstacleForHex(q, r, terrain);
        ...
        return {
            ...
            obstacle: obstacle, // Преграда (null, 'mountain', 'chasm', 'water')
    ...
    /**
     * Генерация преграды рельефа для гекса
     * @param {number} q - Axial-координата q
     * @param {number} r - Axial-координата r
     * @param {string} terrain - Тип местности
     * @returns {string|null} Тип преграды (null, 'mountain', 'chasm', 'water')
     * 
     * [ЧТО] Создаёт горы, пропасти и водные преграды на карте
     * [ЗАЧЕМ] Тактические элементы для стратегического геймплея (план 1.3.1)
     * [PLAN] 1.3.2 - Добавить влияние на сигнал, 1.3.3 - Тактическое использование
     */
    generateObstacleForHex(q, r, terrain) {
        // [ЧТО] Используем детерминированный шум на основе координат
        // [ЗАЧЕМ] Преграды генерируются одинаково при каждом запуске с теми же координатами
        // [PLAN] Добавить настройку частоты преград через параметры
        
        // Базовый шум для определения наличия преграды
        const noise = Math.sin(q * 0.1) * Math.cos(r * 0.1) * 0.5 + 
                      Math.sin(q * 0.3 + r * 0.2) * 0.3 +
                      Math.random() * 0.2;
        
        // [ЧТО] Горы чаще появляются в снежных регионах и на "высоких" значениях шума
        // [ЗАЧЕМ] Естественное распределение - горы в холодных регионах
        // [PLAN] 1.3.1 - Горы блокируют сигнал и строительство
        if (terrain === 'snow' && noise > 0.6) {
            return 'mountain';
        }
        
        // [ЧТО] Пропасти появляются реже, создавая разрывы в карте
        // [ЗАЧЕМ] Тактические узкие места для контроля территории
        // [PLAN] 1.3.1 - Пропасти разрывают связь
        if (noise < -0.7 && Math.random() > 0.5) {
            return 'chasm';
        }
        
        // [ЧТО] Водные преграды чаще в пустынях и на границах биомов
        // [ЗАЧЕМ] Вода блокирует строительство, но не сигнал
        // [PLAN] 1.3.1 - Вода позволяет проходить сигналу
        if (terrain === 'desert' && noise > 0.4 && noise < 0.65) {
            return 'water';
        }
        
        // [ЧТО] Небольшой шанс воды в равнинах near coast
        if (terrain === 'plains' && noise > 0.55 && noise < 0.65 && Math.random() > 0.7) {
            return 'water';
        }
        
        // Преграда отсутствует
        return null;
    }
    
    /**
     * Проверка проходит ли сигнал через гекс
     * @param {Object} hex - Объект гекса
     * @returns {boolean} true если сигнал проходит
     * 
     * [ЧТО] Определяет блокирует ли гекс прохождение сигнала
     * [ЗАЧЕМ] 1.3.2 - Сигнал не проходит через горы и пропасти
     * [PLAN] 1.3.3 - Тактическое использование для защиты сети
     */
    isSignalPassable(hex) {
        if (!hex) return false;
        
        // [ЧТО] Горы и пропасти блокируют сигнал
        // [ЗАЧЕМ] 1.3.2 - Влияние на геймплей
        if (hex.obstacle === 'mountain' || hex.obstacle === 'chasm') {
            return false;
        }
        
        // [ЧТО] Вода пропускает сигнал
        // [ЗАЧЕМ] 1.3.1 - Водные преграды не блокируют связь
    ...
    /**
     * Проверка можно ли построить здание на гексе
     * @param {Object} hex - Объект гекса
     * @returns {boolean} true если строительство возможно
     * 
     * [ЧТО] Определяет доступно ли размещение постройки на гексе
     * [ЗАЧЕМ] 1.2.3 - Логика размещения на биомах, 1.3.1 - Учёт преград
     * [PLAN] 1.3.3 - Тактическое использование рельефа
     */
    canBuildOn(hex) {
        if (!hex) return false;
        
        // [ЧТО] Проверяем наличие преграды
        // [ЗАЧЕМ] 1.3.1 - Горы, вода и пропасти блокируют строительство
        // [PLAN] 1.3.3 - Некоторые постройки можно строить только на горах (радары)
        if (hex.obstacle === 'mountain' || hex.obstacle === 'chasm' || hex.obstacle === 'water') {
            return false;
        }
        
        // [ЧТО] Проверяем наличие другой постройки
        // [ЗАЧЕМ] Нельзя строить поверх существующей постройки
        // [PLAN] Добавить возможность улучшения построек
        if (hex.building !== null) {
            return false;
        }
        
        // [ЧТО] Все биомы доступны для строительства (равнины, пустыня, снег)
        // [ЗАЧЕМ] 1.2.3 - Пользователи и постройки могут размещаться на любом типе региона
        // [PLAN] Добавить ограничения для специальных типов построек
    ...
    placeBuilding(hexId, building) {
        ...
        // [ЧТО] Дополнительная проверка возможности строительства
        // [ЗАЧЕМ] 1.3.1 - Блокировка строительства на преградах
        // [PLAN] 1.3.3 - Специальные правила для разных типов местности
        if (!this.canBuildOn(hex)) {
            console.log(`[HexGrid] Нельзя строить на гексе ${hexId}: препятствие ${hex.obstacle || 'нет места'}`);
            return false;
        }
